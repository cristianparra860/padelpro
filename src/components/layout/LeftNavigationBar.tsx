...
export function LeftNavigationBar() {
    ...
    return (
        <div className="fixed left-0 top-1/2 -translate-y-1/2 z-40 flex flex-col gap-3 items-center pl-1 md:hidden">
                ...
                return (
                    <div key={item.key} className="flex flex-col items-center gap-1">
                        ...
                        {isClasesButton && item.isActive && (
                            <button
                                onClick={scrollToTop}
                                className="w-6 h-6 rounded-full bg-white border border-green-500 shadow-[inset_0_1px_3px_rgba(34,197,94,0.2)] flex items-center justify-center mb-1 hover:scale-110 transition-transform"
                                ...
                                >
                                    {item.showAvatar && currentUser?.profilePictureUrl ? (
                                        <Avatar className="h-7 w-7">
                                            ...
                                            <AvatarFallback className="text-xs">{getInitials(currentUser.name || 'U')}</AvatarFallback>
                                        </Avatar>
                                    ) : (
                                        <IconComponent 
                                            className={cn(
                        ...
                        <span 
                            className={cn(
                                "text-[8px] font-medium uppercase tracking-wide transition-colors",
                    ...
                    {isAgendaButton && item.isActive && (
                        <button
                            ...
                            className="w-6 h-6 rounded-full bg-white border border-green-500 shadow-[inset_0_1px_3px_rgba(34,197,94,0.2)] flex items-center justify-center mt-1 hover:scale-110 transition-transform"
                            aria-label="Ir abajo"
                            title="Ir al final"
                        >
